...
```text
请从 https://github.com/kalisam2015/new-canvas.git 安装 Infinite Canvas Codex 插件。
请 clone 仓库到 ~/plugins/new-canvas，确认 plugins/infinite-canvas/.codex-plugin/plugin.json 存在，
...
```bash
mkdir -p ~/plugins
git clone https://github.com/kalisam2015/new-canvas.git ~/plugins/new-canvas
```
  ...
  "plugins": [
    {
      ...
      "source": {
        "source": "local",
        "path": "./plugins/new-canvas/plugins/infinite-canvas"
      },
      "policy": {
